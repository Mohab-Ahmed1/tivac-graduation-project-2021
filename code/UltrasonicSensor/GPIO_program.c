...
//        CRPORT_C    = 0x0FF ;                       /* Commit register */
        ...
        break;
    case PORTD :
        //LOCKPORT_D |= __Unlock_Register;            /*Unlock GPIO A to be able to write on it */
        //CRPORT_D   = 0x0FF ;                        /* Commit register */
        AFSELPORT_D &= ~(1<<0) & ~(1<<1) & ~(1<<2) & ~(1<<3);                        /*Disable alternative function*/
        DENPORT_D    |= (1<<0) | (1<<1) | (1<<2) |(1<<3);                        /*GPIO Digital Enable*/
        break;
    case PORTE :
...
void Set_PORT_direction(u8 port_number,u8 port_direction)
{
    ...
    case PORTC :
        ...
        break;
    case PORTD :  DIRPORT_D |=0x0F;           break;
